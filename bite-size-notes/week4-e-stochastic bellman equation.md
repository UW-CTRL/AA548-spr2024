...
where $V_\pi(x_k) = \mathbb{E} \[ g_k(x_k) + V_{\pi^*}(x_{k+1}) \]$ referring one-step look ahead optimal cost and $\Pi$ is the set of all __admissible__ policies at state $x_k$.
...
- For finding suboptimal policy, approximate the cost-go-function $\bar{J}^*_{k}$(using Model Approximation, Neural-Nets, Rollout algorithm, Model Predictive Control..etc)

$$
   \bar{V_k}^{\*} = \min_{u_k \in U_k (x_k)} \mathbb{E} \[ g_k(x_k, u_k, w_k) + \bar{V}^*_{k+1}(f_k(x_k, u_k, w_k)) \
$$

where suboptimal policy obtained 
...
However, the DP algorithm can be very time-consuming, but more than the deterministic case since it involves the expected value operation and a rapid increase of the required computation and memory storage as the problem’s size increases. This difficulties motivate suboptimal control schemes, such as approximation in value space whereby we replace $J_k^\*$ with easier obtainable approximation $\bar{J}_k$.  From the looking forward minimization of cost function, we obtain the suboptimal(heuristic) policies $\bar{\mu}$ which are used in situations where finding an optimal solution is computationally intractable or impractical.
